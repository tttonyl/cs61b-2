...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    for(int i = 0; i < ints.length-1; i++) {
      int numberOfItemsToSmoosh = 0;
      int indexToCompare = i+1;

      // count the number of similar items that we will be getting rid of.
      while(indexToCompare < ints.length && ints[i] == ints[indexToCompare]) {
        numberOfItemsToSmoosh++;
        indexToCompare++;
      }

      // shift array to the left and add a -1 at the end for each item that needs to be smooshed.
      while(numberOfItemsToSmoosh > 0) {
        for(int k = i; k < ints.length-1; k++) {
          ints[k] = ints[k+1]; 
        }
